...
from backbone_hdc import resnet50_fpn_backbone
from my_dataset import maskDetectionDataset
...
def main(parser_data):
    ...
    val_dataset = maskDetectionDataset(VOC_root, data_transform["valid"], "valid.txt")
    ...
    # accumulate predictions from all JPEGImages
    ...
    print(print_voc)
    map_name = "record_resnet50hdc_NotAug_mAP-" + parser_data.weights.split("/")[-1].split(".")[0].split("-")[-1] + ".txt"
    # 将验证结果保存至txt文件中
...
if __name__ == "__main__":
    ...
    # 数据集的根目录
    parser.add_argument('--data-path', default='./MaskDatasets_NotAugment', help='dataset root')

    # 训练好的权重文件
    # TODO CHANGE THIS
    parser.add_argument('--weights', default='./save_weights/resNetHDCFpn-model-NotAugment-19.pth', type=str, help='training weights')
